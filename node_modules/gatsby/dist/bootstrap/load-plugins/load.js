...
function resolvePlugin(pluginName) {
  ...
  if (!fs.existsSync(pluginName)) {
    ...
    if (fs.existsSync(resolvedPath)) {
      console.log("local, resolved path is", resolvedPath);
  ...
  try {
    ...
    console.log("resolved path, node module", resolvedPath);

    if (pluginName === "gatsby-remark-vega") {
      console.log("this will fail, since it is looking in the dist directory");
    }
    const packageJSON = JSON.parse(
      fs.readFileSync(`${resolvedPath}/package.json`, `utf-8`)
    )
